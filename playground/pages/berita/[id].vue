<template>
  <div class="min-h-screen bg-slate-50 text-slate-800 font-sans selection:bg-knpi-500 selection:text-white flex flex-col">
    ...
    <main class="flex-1 flex flex-col">
      ...
      <div
        v-if="memuat"
        class="flex flex-col items-center justify-center py-32 mt-10"
      >
        <div class="w-12 h-12 border-4 border-[#003399] border-t-transparent rounded-full animate-spin mb-4" />
      ...
      <div
        v-else-if="error"
        class="container mx-auto max-w-4xl text-center py-32 mt-10 bg-white rounded-3xl shadow-sm border border-slate-100"
      >
        <LucideAlertCircle
          :size="64"
          class="mx-auto text-red-400 mb-6"
        />
        <h2 class="text-2xl font-bold text-slate-900 mb-2">
          Berita Tidak Ditemukan
        </h2>
        <p class="text-slate-500 mb-8">
          Berita yang Anda cari mungkin sudah dihapus atau URL tidak valid.
        </p>
        <NuxtLink
          to="/"
          class="inline-flex items-center gap-2 rounded-full bg-[#003399] px-6 py-3 text-sm font-bold text-white shadow-lg hover:-translate-y-1 hover:bg-blue-800 transition duration-300"
        >
          <LucideHome :size="18" />
          Kembali ke Beranda
        </NuxtLink>
      ...
      >
        ...
        <div class="relative w-full h-[50vh] min-h-[350px] md:h-[60vh] md:min-h-[450px] bg-slate-900 overflow-hidden">
          ...
          <!-- Gradient Overlay -->
          <div class="absolute inset-0 bg-gradient-to-t from-[#003399] via-[#003399]/40 to-transparent" />
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-5xl w-full relative z-10 -mt-10 md:-mt-16 mb-20 animate-fade-in-up delay-300 opacity-0">
          <article class="bg-white rounded-[1.5rem] md:rounded-[2rem] shadow-2xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
            <div class="p-6 sm:p-8 md:p-10 lg:p-12">
              ...
              <!-- Main Content Body -->
              <div
                class="prose prose-base sm:prose-lg prose-slate max-w-none text-slate-700 leading-loose"
            ...
            <div class="bg-slate-50 p-6 md:p-8 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <div class="flex items-center gap-3">
                ...
                <button
                  class="w-10 h-10 rounded-full bg-sky-100 text-sky-500 flex items-center justify-center hover:bg-sky-500 hover:text-white transition-colors"
                  @click="bagikan('twitter')"
                >
                  <LucideTwitter :size="18" />
                </button>
                <button
                  class="w-10 h-10 rounded-full bg-slate-200 text-slate-600 flex items-center justify-center hover:bg-slate-600 hover:text-white transition-colors"
                  @click="salinLink"
                >
                  <LucideLink :size="18" />
                </button>
...
onMounted(async () => {
  ...
  try {
    const res = await $fetch<{ berhasil: boolean, data: Berita }>(`/api/publik/berita/${id}`)
    if (res.berhasil) {
