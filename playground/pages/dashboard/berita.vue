  ...
  <div class="flex flex-col gap-6">
    ...
    <div class="glass-card p-4 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
      ...
      <div class="relative flex-1 min-w-0">
        ...
        <input v-model="kataKunciCari" type="text" placeholder="Cari judul berita atau kata kunci..."
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <div class="flex items-center gap-1 bg-slate-900/60 p-1 rounded-xl border border-white/5 ml-auto md:ml-0">
          <button class="p-1.5 rounded-lg text-xs font-semibold transition cursor-pointer"
            :class="tampilanMode === 'grid' ? 'bg-knpi-500/20 text-knpi-300 border border-knpi-500/30' : 'text-slate-400 hover:text-white'"
            title="Tampilan Kartu Grid" @click="tampilanMode = 'grid'">
          ...
          <button class="p-1.5 rounded-lg text-xs font-semibold transition cursor-pointer"
            ...
            title="Tampilan Tabel Data" @click="tampilanMode = 'tabel'">
            <LucideList :size="15" />
          </button>
        </div>
      </div>
    ...
    <!-- ====== EMPTY STATE ====== -->
    <div v-else-if="beritaFiltered.length === 0"
    ...
    <div v-else-if="tampilanMode === 'grid'" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
      <div v-for="item in beritaFiltered" :key="item.id"
        ...
        <div>
          ...
          <div class="relative w-full aspect-[16/10] overflow-hidden bg-slate-900 border-b border-white/10">
            <img v-if="item.gambarUrl" :src="item.gambarUrl" :alt="item.judul"
            ...
            <button type="button"
              ...
              title="Baca Pratinjau Artikel" @click="bukaPratinjauArtikel(item)">
              <span
        ...
        <div
          ...
          <div class="flex items-center gap-1.5">
            <!-- Toggle Status -->
            <button class="p-1.5 rounded-lg text-xs font-semibold border transition cursor-pointer" :class="item.status === 'terbit'
              ? 'border-slate-500/20 bg-slate-500/10 text-slate-400 hover:bg-slate-500/20'
              : 'border-emerald-500/30 bg-emerald-500/10 text-emerald-300 hover:bg-emerald-500/20'"
              :title="item.status === 'terbit' ? 'Ubah ke Draf' : 'Terbitkan Berita'" @click="toggleStatusBerita(item)">
            ...
            <button
              ...
              title="Edit Berita" @click="bukaFormEdit(item)">
            ...
            <button
              ...
              title="Hapus Berita" @click="konfirmasiHapus(item)">
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left text-sm">
          ...
          <tbody class="divide-y divide-white/[0.04]">
            <tr v-for="item in beritaFiltered" :key="'tbl-' + item.id" class="transition-colors hover:bg-white/[0.02]">
              <!-- Sampul & Judul -->
              <td class="px-5 py-4">
                <div class="flex items-center gap-3 min-w-[280px] max-w-md">
                  ...
                  <div class="min-w-0">
                    <h4 class="font-bold text-white text-xs sm:text-sm truncate hover:text-knpi-300 cursor-pointer"
              ...
              <td class="px-5 py-4 whitespace-nowrap">
                <span class="inline-block rounded-md px-2 py-0.5 text-[10px] font-bold uppercase border"
                  :class="kelasKategori(item.kategori)">
                  {{ labelKategori(item.kategori) }}
                </span>
              </td>
              ...
              <!-- Status -->
              <td class="px-5 py-4 whitespace-nowrap">
                <span class="inline-flex items-center gap-1.5 text-[10px] font-bold px-2.5 py-0.5 rounded-full border"
              ...
              <td class="px-5 py-4 text-right whitespace-nowrap">
                <div class="flex items-center justify-end gap-1.5">
                  <button
                    ...
                    title="Baca Pratinjau" @click="bukaPratinjauArtikel(item)">
      ...
      <div v-if="itemPratinjau"
        ...
        <div class="glass-card w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden shadow-2xl relative">
          ...
          <div class="flex items-center justify-between px-6 py-4 border-b border-white/[0.08] bg-slate-900/80">
            ...
            <button class="p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-white/10 transition cursor-pointer"
          ...
          <div class="flex-1 overflow-y-auto p-6 space-y-4">
            ...
            <div class="flex items-center gap-2">
              <span class="rounded-lg px-2.5 py-1 text-[10px] font-bold uppercase border"
            ...
            <!-- Cover Image -->
            <div v-if="itemPratinjau.gambarUrl"
    ...
    <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 scale-95"
      ...
      <div v-if="tampilForm"
        ...
        <div class="glass-card w-full max-w-5xl max-h-[92vh] flex flex-col overflow-hidden shadow-2xl relative">
          ...
          <form class="flex-1 overflow-y-auto p-6 flex flex-col justify-between gap-6" @submit.prevent="simpanBerita">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              ...
              <div class="lg:col-span-7 flex flex-col gap-4">
                ...
                <div>
                  ...
                  <input v-model="form.judul" type="text" placeholder="Masukkan judul artikel berita"
                ...
                <div>
                  ...
                  <textarea v-model="form.ringkasan" rows="2" placeholder="Masukkan ringkasan singkat artikel"
                ...
                <div>
                  ...
                  <div
                    ...
                    <button type="button"
                      ...
                      title="Batal Tindakan (Undo Ctrl+Z)" @click="lakukanUndo">
                      <LucideUndo2 :size="15" />
                    </button>
                    <button type="button"
                      class="p-1.5 rounded-lg transition cursor-pointer text-xs font-bold flex items-center gap-1"
                      ...
                      title="Ulangi Tindakan (Redo Ctrl+Y)" @click="lakukanRedo">
                      <LucideRedo2 :size="15" />
                    </button>
                    ...
                    <button type="button"
                      ...
                      title="Tebal (Bold <b>)" @click="sisipkanFormat('<b>', '</b>')">
                      <LucideBold :size="15" />
                    </button>
                    <button type="button"
                      class="p-1.5 rounded-lg hover:bg-white/10 hover:text-white transition cursor-pointer text-xs font-bold flex items-center gap-1"
                      title="Miring (Italic <i>)" @click="sisipkanFormat('<i>', '</i>')">
                      <LucideItalic :size="15" />
                    </button>
                    <button type="button"
                      class="p-1.5 rounded-lg hover:bg-white/10 hover:text-white transition cursor-pointer text-xs font-bold flex items-center gap-1"
                      title="Garis Bawah (Underline <u>)" @click="sisipkanFormat('<u>', '</u>')">
                      <LucideUnderline :size="15" />
                    </button>
                    <div class="h-4 w-[1px] bg-white/10 mx-1" />
                    <button type="button"
                      class="p-1.5 rounded-lg hover:bg-white/10 hover:text-white transition cursor-pointer text-xs font-bold flex items-center gap-1"
                      title="Subjudul (Heading <h3>)" @click="sisipkanFormat('\n<h3>', '</h3>\n')">
                      <LucideHeading :size="15" />
                    </button>
                    <button type="button"
                      class="p-1.5 rounded-lg hover:bg-white/10 hover:text-white transition cursor-pointer text-xs font-bold flex items-center gap-1"
                      title="Poin List (Bullet •)" @click="sisipkanFormat('\n• ')">
                      <LucideList :size="15" />
                    </button>
                    <button type="button"
                      class="p-1.5 rounded-lg hover:bg-white/10 hover:text-white transition cursor-pointer text-xs font-bold flex items-center gap-1"
                      title="Angka List (Numbered 1.)" @click="sisipkanFormat('\n1. ')">
                    ...
                    <button type="button"
                      class="p-1.5 rounded-lg hover:bg-white/10 hover:text-white transition cursor-pointer text-xs font-bold flex items-center gap-1"
                      title="Garis Pemisah (<hr>)" @click="sisipkanFormat('\n<hr>\n')">
                      <LucideMinus :size="15" />
                    </button>
                  </div>

                  <!-- Textarea Konten Kebawah -->
                  <textarea ref="refKontenTextarea" v-model="form.konten" rows="10"
              ...
              <div class="lg:col-span-5 flex flex-col gap-4">
                <div>
                  ...
                  <!-- Preview Gambar -->
                  <div v-if="previewGambar || form.gambarUrl"
                    ...
                    <div
                      ...
                      <button type="button"
                        ...
                        title="Ganti Gambar Sampul" @click="hapusGambar">
                  ...
                  <div v-if="!previewGambar && !form.gambarUrl"
                    ...
                    @click="($refs.inputGambar as HTMLInputElement)?.click()" @dragover.prevent @drop.prevent="handleDrop">
                    ...
                    <p class="text-sm font-bold text-slate-200">Klik atau seret file gambar sampul ke sini</p>
                    ...
                    <input ref="inputGambar" type="file" accept="image/jpeg,image/png,image/webp,image/gif" class="hidden"
                      @change="handleFileChange" />
                  </div>

                  <!-- Loading Upload -->
                  <div v-if="sedangUpload" class="mt-3 flex items-center justify-center gap-2 text-xs font-semibold text-knpi-400 p-3 rounded-xl bg-knpi-500/10 border border-knpi-500/20">
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-white/10">
              ...
              <button type="submit" :disabled="sedangMenyimpan || sedangUpload"
    ...
    <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 scale-95"
      leave-active-class="transition duration-150 ease-in" leave-to-class="opacity-0 scale-95">
      <div v-if="tampilKonfirmasiHapus"
        ...
        <div class="glass-card w-full max-w-sm p-6 text-center relative overflow-hidden shadow-2xl">
          ...
          <div class="mt-6 flex gap-3">
            ...
            <button
              ...
              :disabled="sedangMenghapus" @click="hapusBerita">
    ...
    <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0 translate-y-4"
      ...
      <div v-if="toast.tampil"
        ...
        <LucideAlertCircle v-if="toast.tipe === 'error'" :size="18" />
        <LucideCheckCircle v-else :size="18" />
...
let timerDebounceRiwayat: any = null
