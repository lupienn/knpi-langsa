  ...
  <div class="flex flex-col gap-6">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
      ...
      <div class="glass-card p-4 flex items-center gap-3.5 border-knpi-500/20">
        ...
        <div>
          <p class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Total Berita</p>
          <p class="text-xl font-extrabold text-white mt-0.5">{{ daftarBerita.length }}</p>
        </div>
      </div>
      ...
      <div class="glass-card p-4 flex items-center gap-3.5 border-emerald-500/20">
        ...
        <div>
          <p class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Terpublikasi</p>
          <p class="text-xl font-extrabold text-emerald-400 mt-0.5">{{ totalTerbit }}</p>
        </div>
      </div>
      ...
      <div class="glass-card p-4 flex items-center gap-3.5 border-amber-500/20">
        ...
        <div>
          <p class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Draf Disimpan</p>
          <p class="text-xl font-extrabold text-amber-400 mt-0.5">{{ totalDraf }}</p>
        </div>
      </div>
      ...
      <div class="glass-card p-4 flex items-center gap-3.5 border-violet-500/20">
        ...
        <div>
          ...
          <p class="text-xl font-extrabold text-violet-400 mt-0.5">{{ totalKegiatan }}</p>
        </div>
      </div>
    </div>
    ...
    <div v-if="sedangMemuat" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <div v-for="i in 6" :key="'skel-' + i" class="glass-card p-4 flex flex-col gap-3 animate-pulse">
    ...
    >
      <div class="w-16 h-16 rounded-2xl bg-knpi-500/10 border border-knpi-500/20 flex items-center justify-center text-knpi-400 shadow-inner">
        <LucideNewspaper :size="32" />
      </div>
      <div>
        <h3 class="text-white font-bold text-lg">Tidak Ada Berita Ditemukan</h3>
        <p class="text-slate-400 text-sm mt-1 max-w-md">
          {{ kataKunciCari ? `Tidak ada berita yang cocok dengan kata kunci "${kataKunciCari}".` : 'Belum ada berita yang diterbitkan. Klik tombol di bawah untuk membuat berita baru.' }}
        </p>
      </div>
      <button
    ...
    <!-- ====== TAMPILAN GRID EDITORIAL (DEFAULT) ====== -->
      ...
      >
        <div>
          ...
          <div class="relative w-full aspect-[16/10] overflow-hidden bg-slate-900 border-b border-white/10">
            ...
            >
              <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/20 border border-white/30 backdrop-blur-md text-xs font-bold shadow-lg">
          ...
          <div class="p-4 sm:p-5">
            <h3 class="font-bold text-white text-base line-clamp-2 leading-snug group-hover:text-knpi-300 transition-colors">
              {{ item.judul }}
            </h3>
        ...
        <!-- Card Footer Toolbar -->
        <div class="px-4 sm:px-5 pb-4 sm:pb-5 pt-3 border-t border-white/[0.06] flex items-center justify-between gap-2">
          <!-- Meta Date -->
          <div class="flex items-center gap-1.5 text-[11px] font-medium text-slate-400">
            <LucideCalendar :size="13" class="text-slate-500" />
          ...
          <div class="flex items-center gap-1.5">
            ...
            >
              <LucideEyeOff v-if="item.status === 'terbit'" :size="14" />
        ...
        <table class="w-full text-left text-sm">
          <thead>
            <tr class="border-b border-white/[0.08] bg-white/[0.02] text-[10px] font-bold uppercase tracking-widest text-slate-400">
              <th class="px-5 py-3.5">Berita & Ringkasan</th>
            ...
            >
              ...
              <td class="px-5 py-4">
                <div class="flex items-center gap-3 min-w-[280px] max-w-md">
                  ...
                  <div class="min-w-0">
                    <h4 class="font-bold text-white text-xs sm:text-sm truncate hover:text-knpi-300 cursor-pointer" @click="bukaPratinjauArtikel(item)">
                      {{ item.judul }}
                    </h4>
                    <p class="text-[11px] text-slate-400 truncate mt-0.5">{{ item.ringkasan }}</p>
                  </div>
                </div>
              </td>
              ...
              <td class="px-5 py-4 whitespace-nowrap">
                <span class="inline-block rounded-md px-2 py-0.5 text-[10px] font-bold uppercase border" :class="kelasKategori(item.kategori)">
                  {{ labelKategori(item.kategori) }}
                </span>
              </td>
        ...
        <div class="glass-card w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden shadow-2xl relative">
          ...
          <div class="flex-1 overflow-y-auto p-6 space-y-4">
            ...
            <div class="flex items-center gap-2">
              <span class="rounded-lg px-2.5 py-1 text-[10px] font-bold uppercase border" :class="kelasKategori(itemPratinjau.kategori)">
            ...
            <!-- Cover Image -->
            <div v-if="itemPratinjau.gambarUrl" class="rounded-2xl overflow-hidden aspect-[16/9] bg-slate-900 border border-white/10 my-3">
              <img :src="itemPratinjau.gambarUrl" :alt="itemPratinjau.judul" class="w-full h-full object-cover" />
            </div>

            <!-- Excerpt Blockquote -->
            <div class="p-4 rounded-xl bg-white/[0.03] border-l-4 border-knpi-500 text-sm italic text-slate-300 leading-relaxed">
              "{{ itemPratinjau.ringkasan }}"
            </div>

            <!-- Content Paragraphs -->
            <div class="text-xs sm:text-sm text-slate-200 leading-relaxed space-y-3 whitespace-pre-line border-t border-white/5 pt-4">
              {{ itemPratinjau.konten }}
            </div>
          </div>
        </div>
      </div>
    ...
    >
      ...
      >
        <div class="glass-card w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl relative">
          <!-- Modal Header -->
            ...
            <div class="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p class="text-[11px] text-slate-400">
                  {{ modeEdit ? 'Perbarui konten artikel berita yang telah ada' : 'Isi formulir editorial untuk publikasi warta DPD KNPI Langsa' }}
                </p>
              </div>
            </div>
            <button
          ...
          <!-- Modal Body Form -->
          <form class="flex-1 overflow-y-auto p-6 flex flex-col gap-4" @submit.prevent="simpanBerita">
            <!-- Judul -->
            <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="terbit" class="bg-slate-900 text-white">Terbit (Publikasi)</option>
                </select>
              </div>
            </div>
            ...
            <div>
              <div class="flex items-center justify-between mb-1.5">
                ...
                <span class="text-[10px] text-slate-500">{{ form.konten.length }} Karakter</span>
              </div>
              <textarea
                v-model="form.konten"
                rows="6"
                placeholder="Tuliskan isi konten berita secara lengkap di sini..."
                class="w-full rounded-xl border border-white/10 bg-slate-900/80 px-4 py-2.5 text-sm text-white placeholder:text-slate-500 focus:border-knpi-500 focus:outline-none focus:ring-1 focus:ring-knpi-500/50 resize-y transition-all"
                required
              />
            </div>
            ...
            <div>
              ...
              <!-- Preview Gambar -->
              <div v-if="previewGambar || form.gambarUrl" class="relative rounded-xl overflow-hidden aspect-[16/9] bg-slate-900 border border-white/15 shadow-inner group">
              ...
              <div
                ...
                class="relative flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-white/15 bg-slate-900/50 py-7 px-4 text-center hover:border-knpi-500/50 hover:bg-knpi-500/[0.02] transition-all cursor-pointer"
                @click="($refs.inputGambar as HTMLInputElement)?.click()"
                @dragover.prevent
                @drop.prevent="handleDrop"
              >
                <div class="p-3 rounded-full bg-knpi-500/10 text-knpi-400 mb-2 border border-knpi-500/20">
                  <LucideUploadCloud :size="24" />
                </div>
                <p class="text-xs font-bold text-slate-200">Klik atau seret file gambar sampul ke sini</p>
              ...
              <!-- Loading Upload -->
              <div v-if="sedangUpload" class="mt-2.5 flex items-center gap-2 text-xs font-semibold text-knpi-400">
            ...
            <div class="flex gap-3 pt-2">
              ...
              <button
                ...
                class="flex flex-1 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-knpi-600 to-knpi-500 py-2.5 text-xs font-bold text-white shadow-lg shadow-knpi-500/20 hover:brightness-110 disabled:opacity-50 disabled:cursor-not-allowed transition cursor-pointer"
              >
                <LucideLoader v-if="sedangMenyimpan" :size="15" class="animate-spin" />
                <LucideCheck v-else :size="15" />
                <span>{{ sedangMenyimpan ? 'Menyimpan...' : (modeEdit ? 'Simpan Perubahan' : 'Terbitkan Berita') }}</span>
              </button>
            </div>
          </form>
        </div>
    ...
    >
      ...
      >
        <div class="glass-card w-full max-w-sm p-6 text-center relative overflow-hidden shadow-2xl">
          ...
          <h3 class="text-base font-bold text-white">Hapus Berita Ini?</h3>
          <p class="mt-1.5 text-xs leading-relaxed text-slate-400">
            Berita <span class="text-white font-semibold">"{{ beritaDihapus?.judul }}"</span> akan dihapus secara permanen dari sistem.
          </p>
